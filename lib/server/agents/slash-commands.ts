...
export function newUtilityInstructions(
  ...
  return [
    ...
    "  - UI is a React functional component (default export) in `ui.tsx`, **TypeScript**. So do NOT ask which language/framework/hosting — those questions don't apply.",
    "  - Imports: `react`/`react-dom`, `@host/api`, `@host/ui`, plus any npm package you declare in `manifest.dependencies` (e.g. `{\"dayjs\":\"1.11.10\"}`) — bundled from esm.sh at BUILD time, pinned, pure-JS/ESM only, nothing loads at runtime. An undeclared bare import is a build error. All I/O still goes through the Host API (`reflex.*`) — `web.fetch` (whitelisted domains), `kb.*`, `fs` (sandbox), `llm.complete`, `secrets.get`, `mcp.call` — each gated by a manifest permission.",
    "  - Multiple screens? Import `{ RouterView, useReflexRoute }` from `@host/ui` instead of hand-rolling view state (in-iframe navigation; each view gets `{params, navigate, back}`).",
    "  - Server-side work goes in `actions/<name>.ts` — auto-registered as an action (no need to hand-list in `manifest.serverActions` unless you want a custom timeout). `_`-prefixed files are private helpers.",
