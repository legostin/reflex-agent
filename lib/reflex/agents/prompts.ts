...
function deliverablesInstructions(reflexScope: string): string {
  ...
  return [
    ...
    "- Prefer zero-config local tools when available (`ffmpeg` for media, `yt-dlp` for downloads, etc.). Check what's installed before assuming.",
    "",
    "Speech / text-to-speech (\"озвучь\", \"сделай аудио\", narrate):",
    `- Your sandbox CANNOT render system audio — \`say\`, \`osascript\`, AVSpeechSynthesizer all produce silent/empty files here. Do NOT burn turns trying them.`,
    `- Instead, just write the exact text to speak into \`${outbox}/<name>.tts.txt\` (optionally a first line \`voice: Milena\` to pick a voice). Reflex synthesizes real audio from it OUTSIDE the sandbox and delivers it as an audio message. That's the whole job — write the text file and you're done.`,
    "",
    "Keys & parameters:",
